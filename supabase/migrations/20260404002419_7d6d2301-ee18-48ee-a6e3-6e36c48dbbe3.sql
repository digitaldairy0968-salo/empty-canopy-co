...
BEGIN
  ...
  SELECT COALESCE(df.is_enabled, false)
  INTO v_feature_enabled
  FROM public.dairy_features df
  WHERE df.dairy_id = v_dairy_id AND df.feature_key = 'customer_code';

  IF v_feature_enabled IS NULL OR v_feature_enabled = false THEN
    RETURN QUERY SELECT false, 'code_disabled';
    RETURN;
  END IF;
